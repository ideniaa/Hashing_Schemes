...
public:
	DoubleHashing(string filename, int capacity);			// constructor
...
protected:
	...
	int lastPrime();									// get last prime number before N
private:
...
public:
	...
	public:
		...
		friend class DoubleHashing;						// give class acceess
	};
};

template <typename K, typename V, typename W>
DoubleHashing<K, V, W>::DoubleHashing(string filename, int capacity) : n(0), B(capacity) // constructor
{
...
template <typename K, typename V, typename W>
int DoubleHashing<K, V, W>::size() const { return n; }	// number of entries

template <typename K, typename V, typename W>
bool DoubleHashing<K, V, W>::empty() const { return size() == 0; } // is map empty?

template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Entry& DoubleHashing<K, V, W>::Iterator::operator*() const
...
bool DoubleHashing<K, V, W>::Iterator::operator==(Iterator p) const
{
...
template <typename K, typename V, typename W>
bool DoubleHashing<K, V, W>::Iterator::operator!=(const Iterator& p) const
{
...
typename DoubleHashing<K, V, W>::Iterator& DoubleHashing<K, V, W>::Iterator::operator++()
{
	++ent;								// next entry in bucket array
	while (ent->key() == NULL && ent->value1() == NULL &&  ent != ba->end() || ent->key() == -1)
...
template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Iterator DoubleHashing<K, V, W>::end()
{
	return Iterator(B.end(), B);	// iterator to end
}

template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Iterator DoubleHashing<K, V, W>::begin()
{
...
template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Iterator DoubleHashing<K, V, W>::finder(K k)// find utility
{
	...
	while ((*p).key() != NULL && (*p).key() != k || (*p).key() == -1) { // search for k
		i = (i + f) % B.size();		// get next index to search
...
template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Iterator DoubleHashing<K, V, W>::find(K k)// find key
{
...
template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Iterator DoubleHashing<K, V, W>::inserter(Iterator p, Entry e)
{
...
template <typename K, typename V, typename W>
typename DoubleHashing<K, V, W>::Iterator DoubleHashing<K, V, W>::put(K k, V v, W w)
{
...
template <typename K, typename V, typename W>
void DoubleHashing<K, V, W>::eraser(Iterator p)
{
...
template <typename K, typename V, typename W>
void DoubleHashing<K, V, W>::erase(Iterator p)
{
	eraser(p);	// remove entry at p
}

template <typename K, typename V, typename W>
void DoubleHashing<K, V, W>::erase(K k) {	// remove entry with key k
...
template <typename K, typename V, typename W>
void DoubleHashing<K, V, W>::printReport()
{
...
template <typename K, typename V, typename W>
void DoubleHashing<K, V, W>::printRecords()	// display all records
{
...
template<typename K, typename V, typename W>
void DoubleHashing<K, V, W>::printPos(Iterator p)
{
	if (p != end())
...
template <typename K, typename V, typename W>
int DoubleHashing<K, V, W>::lastPrime()		// get largest prime number smaller than size (q < N)
{
	int count = 0;
